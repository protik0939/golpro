...
export default function Bookmarks() {
  const [bookmarkedContents, setBookmarkedContents] = useState<IContent[] | null>(null);
  ...
  useEffect(() => {
    const fetchBookmarkedContents = async () => {
      try {
        const res = await fetch("/api/contentcrud/byBookmark");
        if (!res.ok) throw new Error("Failed to fetch bookmarked contents");

        const data = await res.json();
        setBookmarkedContents(data.contents);
      } catch (error) {
